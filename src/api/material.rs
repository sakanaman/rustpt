...
{
   fn sample(&self, ray : &Ray, normal : &Vector3, rng : &mut ThreadRng)->(Vector3, Vector3); // return (contribution, ray)
...
impl SampleBSDF for Diffuse {
    fn sample(&self, ray : &Ray, normal : &Vector3, rng : &mut ThreadRng)->(Vector3, Vector3) {
        let u : f64 = rng.gen();
        let r2 : f64 = rng.gen();
        let r1 =  2.0 * PI * u;
        let r2s = f64::sqrt(r2);
